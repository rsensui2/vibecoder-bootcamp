...
```
Cursorに以下のように指示してください：

「ホームページに青いお問い合わせボタンを追加してください。
- 新しいコンポーネント ContactButton.tsx を作成
- src/app/page.tsx にボタンを表示
- Tailwind CSSでスタイリング（bg-blue-500、白テキスト、padding、rounded）」
```

**AIが生成するファイル例**:

```typescript
// src/components/ContactButton.tsx（AIが生成）
...
> **重要**: 上記はAIが生成するコードの一例です。実際の開発では、Cursorに指示を出してコードを生成させましょう。
