...
### ✅ 前提条件（開始前日までに済ませる）
- GitHubアカウントを作成し、SSO/2FAも設定済みであること。学生の方は個人アカウント、企業の方は所属組織の承認を受けたアカウントを使ってください。
- 管理者権限を持つWindows 10/11端末を利用し、Microsoft Store(App Installer)と`winget`が使えるか確認すること。社給PCはIT部門に事前申請しておくとスムーズです。
- Cursorアプリをインストールし、GitHubログイン情報と認証デバイス（Authenticatorやセキュリティキー）を手元に用意しておくこと。
- 安定したネットワークと5GB程度の空き容量を確保し、可能であれば復元ポイントやバックアップを作成してから着手すること。

上記4点がそろえば、学生PCでも企業PCでも同じ流れでステップ0→ステップ1に進めます。VPNやプロキシ環境では`winget`/npm通信を許可してもらい、Docker Desktop + WSL2も先に入れておくと後半のSupabase CLIまで滑らかです。ネイティブモジュールを使う予定がある場合はVisual Studio Build Toolsのインストール権限も確認してください。
...
### 1-0. 実行前のチェック（共通）
1. Cursorを起動するとGitHubのSSO/2FA認証が求められます。学生の方も企業の方も、ブラウザで認証できる状態にしてからComposerを開きましょう。
2. これ以降のPowerShellスクリプトには`Y/N`の質問やGitHubユーザー名/メールの入力が含まれます。画面を閉じずに、指示が出たらすぐ入力できるようにしておいてください。
3. Node.js / pnpm / Gitのインストール直後はPATH反映のためにPowerShellとCursorを再起動する必要があります。フェーズの冒頭行をメモしておき、再起動後にその箇所から再実行するイメージで進めましょう。
...
> **2025年11月更新**: 以下フェーズでは`Y/N`応答・認証が必要になるため、完全放置では進みません。各確認が出たら即座に回答できるように監視してください。
...
Node.jsを入れた直後はPATHが反映されません。PowerShellとCursorを一度閉じるとComposerセッションも終了するので、フェーズ3の冒頭行を控えておき再起動後にそこから再実行してください。wingetのインストールではUAC(ユーザーアカウント制御)の"はい"承認が必要なので、学生PCでも企業PCでもダイアログに気づいたら即承認してください。
...
pnpm導入後も再起動が必要になります。どこまで完了したかメモしてからウィンドウを閉じ、再起動後にフェーズ4冒頭から再開してください。
...
GitHubユーザー名/メールの入力は企業アカウントだと組織ポリシーに従う必要があります。必要なら管理者に確認し、入力値を決めてからフェーズ5を実行してください。
...
Supabase CLIでローカルDBを起動する予定がある場合は、事前にDocker Desktop + WSL2バックエンドを有効化しておきましょう。ここではCLI本体のみを導入します。
...
このあと実行する `pnpm create next-app@latest my-first-app` は対話形式です。完全無人で進めたい場合は `--yes --ts --eslint --tailwind --src-dir --app --turbopack` を併用してください。

pnpm create next-app@latest my-first-app

学生寮や企業ネットワークではポート3000や`node.exe`がセキュリティ製品にブロックされる場合があります。`pnpm dev`で接続できないときは、ファイアウォール例外の申請やポート解放を確認してください。
