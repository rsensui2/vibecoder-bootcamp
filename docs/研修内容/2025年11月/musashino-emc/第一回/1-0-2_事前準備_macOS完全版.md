...
### ✅ 前提条件（前日までに済ませておく）
- GitHubアカウント作成＆SSO/2FA設定が完了していること。学生は個人アカウント、企業メンバーは組織承認済みアカウントを使用してください。
- 管理者権限を持つmacOS（IntelまたはApple Silicon）を使い、App Storeにサインイン済みであること。
- Cursorアプリをインストールし、GitHubログイン情報と認証デバイス（Authenticator等）を手元に準備しておくこと。
- 安定したネットワークと5GB程度の空き容量を確保し、可能ならTime Machineなどでバックアップを作成してから作業すること。

これらが揃えば、学生・企業どちらのMacでも同じ流れでステップ0→ステップ1に進めます。VPNやプロキシ環境では`brew`/npm通信が許可されているか事前に確認するとスムーズです。
...
### 1-0. 実行前のチェック
1. Cursorを開くとGitHubのSSO/2FA認証が求められます。ブラウザや認証アプリを準備してからComposerを起動してください。
2. これ以降のスクリプトには`read`でGitHubユーザー名・メールを入力するステップや`pnpm create`の対話式質問があります。放置せず、指示が出たらすぐ入力しましょう。
3. `pnpm dev`はCtrl+Cで止めるまで戻らないため、動作確認後は自分で停止してください。
...
# 学内LANや企業LANでポート3000がブロックされた場合はファイアウォール例外を申請してください。Ctrl+Cで停止できます。
```
