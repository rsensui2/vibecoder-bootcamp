...
graph TB
    subgraph "認証 (Authentication)"
        A[訪問者] -->|住民カード提示| B[エントランス]
        B -->|本人確認OK| C[マンション内に入れる]
        B -->|本人確認NG| D[入れない]
    end
...
graph TD
    ...
    subgraph "After: Clerk使用"
        A2[Clerkサインアップ] -->|5分| B2[設定]
        B2 -->|10分| C2[実装]
        C2 -->|15分| D2[完成！]
    end
```
...
sequenceDiagram
    ...
    App->>Clerk: 認証開始リクエスト
    Clerk->>Google: OAuth認証要求
    Google->>User: Googleログイン画面
    User->>Google: メール/パスワード入力
    Google->>Google: 認証確認
    Google->>Clerk: 認証トークン返却
    Clerk->>Clerk: ユーザー情報生成
    Clerk->>App: JWT Token発行
    App->>User: ログイン成功！
```
...
sequenceDiagram
    ...
    User->>Next: アクセス
    Next->>Clerk: 認証状態確認
    Clerk->>Next: JWT Token発行

    User->>Next: データ取得要求
    Next->>Next: ClerkのJWT取得
    Next->>Supabase: JWT付きリクエスト
    Supabase->>Supabase: JWT検証
    Supabase->>Supabase: RLSポリシー適用
    Supabase->>Next: ユーザーデータ返却
    Next->>User: 画面表示
```
