...
実装完了後は、私が実行すべき手順（Clerk Dashboard 設定、Supabase 設定、環境変数、Webhook 設定など）を詳細に説明してください。
```
...
実装完了後、Clerk Webhook の設定方法も教えてください。
```
...
6. ローディング・エラー状態の UI
7. Clerk Webhook エンドポイント（ユーザー自動同期）
8. リアルタイム機能の UI（必要な場合）

設計ドキュメントは /docs/ フォルダにあります。
実装完了後、Clerk Webhook の設定手順を詳しく教えてください。
```
